...
import com.ps19.chatwithdocsmobileapp.R
import kotlinx.coroutines.*
import org.json.JSONObject

interface Callback {
    operator fun invoke(result: GoogleSignInResult?)
}
...
class LoginHelper(private val context: Context) {
 ...
 //   @RequiresApi(33)
    ...
    fun createMsalClient(callback: Callback) {
        try {

            val clientApp =  SingleAccountPublicClientApplication.create(context, R.raw.auth_config_ciam_auth);
         //   callback("Client created successfully")
        } catch (e: Exception) {
            print(e.message);
    ...
    companion object {
        ...
        fun invokeHandleLoginMicrosoft(context: Context, clientId: String, callback: Callback) {
            try {
                val helper = LoginHelper(context)
                Log.i("ClientId", clientId)
                helper.createMsalClient(callback);
